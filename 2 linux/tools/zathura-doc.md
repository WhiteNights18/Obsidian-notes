## Mouse and Key Bindings鼠标和按键绑定

## General 概述

| Input 输入              | Description 描述                                                                  |
| --------------------- | ------------------------------------------------------------------------------- |
| J, PgDn               | Go to the next page   前往下一页                                                     |
| K, PgUp               | Go to the previous page   返回上一页                                                 |
| h, k, j, l            | Scroll to the left, down, up or right direction   向左、下、上或右滚动                    |
| Left, Down, Up, Right | Scroll to the left, down, up or right direction   向左、下、上或右滚动                    |
| ^t, ^d, ^u, ^y        | Scroll a half page left, down, up or right   向左、向下、上或右滚动半页                      |
| t, ^f, ^b, space, , y | Scroll a full page left, down, up or right   向左、向下、向上或向右滚动整页                    |
| gg, G, nG             | Goto to the first, the last or to the nth page   翻到第一页、最后一页或第 n 页               |
| P                     | Snaps to the current page   快照到当前页面                                             |
| H, L                  | Goto top or bottom of the current page   点击当前页面的顶部或底部                           |
| ^o, ^i                | Move backward and forward through the jump list   在跳跃列表中前后移动                    |
| ^j, ^k                | Bisect forward and backward between the last two jump points   在最后两个跳跃点之间前后一分为二 |
| ^c, Escape            | Abort 中止                                                                        |
| a, s                  | Adjust window in best-fit or width mode   调整窗口以最佳配合或宽度模式                        |
| /,?                   | Search for text 搜索文本                                                            |
| n, N                  | Search for the next or previous result   搜索下一个或上一个结果                            |
| o, O                  | Open document 开放文档                                                              |
| f                     | Follow links 点击链接                                                               |
| F                     | Display link target 显示链接目标                                                      |
...
| mX                    | Set a quickmark to a letter or number X   将快速标记设置为字母或数字 X。                      |
| 'X                    | Goto quickmark saved at letter or number X   去快速标记，保存在字母或数字 X 处                 |
| q                     | Quit 辞职                                                                         |
...
| Input 输入   | Description 描述                                 |
| ---------- | ---------------------------------------------- |
| J, K       | Go to the next or previous page   请返回下一页或上一页   |
| space,     | Scroll a full page down or up   向下或向上滚动整页      |
...
- */etc/zathurarc*
- *$XDG\_CONFIG\_HOME/zathura/zathurarc* (default: ~/.config/zathura/zathurarc)
...
| adjust\_window | `best-fit`, `width` | Adjust page width. |
| bisect | `forward`, `backward` | Bisect through the document. |
| change\_mode |  | Change current mode. |
| cycle\_first\_column |  | In multiple page layout, cycle the column in which the first page is displayed. |
| display\_link |  | Display link target. |
...
| file\_chooser |  | Open a File Chooser Dialog. Uses the native file chooser on Windows and MacOS, desktop portals when available and otherwise GtkFileChooser. |
...
| navigate\_index | `top`, `bottom`, `up`, `down`, `half-up`, `half-down`, `partial-up`, `partial-down`, `expand`, `collapse`, `expand-all`, `collapse-all`, `toggle`, `select` | Navigate through the index. |
| nohl\_search |  | Remove search highlights. |
...
| set |  | Set an option. |
| snap\_to\_page |  | Snaps to the current page. equivalent to `goto <current_page>` |
| toggle\_fullscreen |  | Toggle fullscreen. |
| toggle\_index |  | Show or hide index. |
| toggle\_inputbar |  | Show or hide inputbar. |
| toggle\_page\_mode |  | Toggle between one and multiple pages per row. |
| toggle\_statusbar |  | Show or hide statusbar. |
| zoom | `in`, `out` | Zoom in or out. |
| mark\_add |  | Set a quickmark. |
| mark\_evaluate |  | Go to a quickmark. |
...
| first-page-column | String | 1:2 | Defines the column in which the first page will be displayed. This setting is stored separately for every value of pages-per-row according to the following pattern <1 page per row>:\[<2 pages per row>\[:...\]\]. The last value in the list will be used for all other number of pages per row if not set explicitly.   Per default, the first column is set to 2 for double-page layout, i.e. the value is set to 1:2. A value of 1:1:3 would put the first page in dual-page layout in the first column, and for layouts with more columns the first page would be put in the 3rd column. |
